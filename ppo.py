...
class DoubleActorCritic(nn.Module):
	def __init__(
			self,
			obs_dim,
			action_dim,
		...
		super(DoubleActorCritic, self).__init__()
		self.device = device
		...
		self.classifier_estimator = nn.Sequential(
			nn.Linear(input_class_dim, hidden_class_dim, dtype=torch.float32),
			nn.Tanh(),
			nn.Linear(hidden_class_dim, 2, dtype=torch.float32),
			nn.Tanh()
		).to(device)

		self.action_var = nn.Parameter(
			torch.full(size=(action_dim,), fill_value=action_std_init * action_std_init)).to(device)
...
class PPOAgent:
	def __init__(
		...
		self.hidden_class_dim = hidden_class_dim
		self.action_std_init = action_std_init
		self.device = device

		self.policy = DoubleActorCritic(
			obs_dim,
			action_dim,
			hidden_dim,
			input_class_dim,
